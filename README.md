...
⚠️ 모든 프로젝트 파일은 올바른 node 버전과 python 버전을 확인 및 설치 후 npm install을 진행 후 사용해주세요.
⚠️ 해당 프로젝트를 테스트하기 위해선 아래와 같은 DB 구조를 가져야합니다. 꼭 참고해주세요.
⚠️ 제작된 라즈베리파이 모듈이 아닌 다른 PC에서 작업하면 파일 경로에 따른 오류가 발생할 수 있습니다.
⚠️ 회원제 서비스를 구성하였기 때문에 최초 아이디와 비밀번호는 업체에서 직접 DB에 등록하여 사용자에게 제공해야합니다.
⚠️ 비밀번호는 반드시 SHA-512 방식으로 암호화후 base64로 변환하여 디비에 등록하고 사용해야합니다.
